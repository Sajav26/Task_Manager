...
const getTasks =async (req, res) => {
    try{
        ...
        //todo Add Completed Task count to each task
...
const updateTaskStatus = async (req, res) => {
    try{
        ...
        if (task.status === 'Completed') {
            task.todoChecklist = task.todoChecklist.map(item => ({ ...item, completed: true }));
            task.progress = 100;
        }
...
const updateTaskChecklist = async (req, res) => {
    try{
        ...
        if (!task.assignedTo.includes(req.user._id) && req.user.role !== 'admin') {
            return res
            .status(403)
            .json({ message: 'You are not authorized to update the checklist of this task' });
        }

        task.todoChecklist = todoChecklist; // Replace with updated checklist

        // Auto-update progres based on checklist completion
        const completedCount = todoChecklist.filter(item => item.completed).length;
        const totalItems = todoChecklist.length;
        task.progress = totalItems> 0 ? Math.round((completedCount / totalItems) * 100) : 0;

        // Auto-mark task as completed if all checked
